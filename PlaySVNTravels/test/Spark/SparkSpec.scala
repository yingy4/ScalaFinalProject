...
import spark.SparkStreamAnalyzer
...
class SparkSpec extends FlatSpec with Matchers {
  ...
  it should "return given number of cheap flights " in {
    val cfs = flightJson.as[CheapFlights]

    val cfsr =  SparkStreamAnalyzer.getCheapestFlights(cfs, 1)
    assert(cfsr.length == 1)

  }

  it should "return maximum number of cheap flights" in {
    val cfs = flightJson.as[CheapFlights]

    val cfsr = SparkStreamAnalyzer.getCheapestFlights(cfs, 3)

    assert(cfsr.length == 2)
  }
